...
public class Mesh {
	...
	void finish_faces_and_sort() {
		...
		for (v = 0; v < vertex_count; ++v) {
			vertices.get(v).face.setVertexAtIndex(vertices.get(v).getIndex(),  vertices.get(v));
		}
	...
	public void add_crease(float p1[], float p2[]) {
		...
		Range resultRange = MeshSmooth.range_for_point(vertices, 0, vertex_count, pp1);
		...
		for (fi = 0; fi < poly_count; ++fi) {
			...
			for (i = 0; i < f.degree; ++i) {
				if (f.neighbor[i] == null) {
					...
					Range result = MeshSmooth.range_for_vertex(vertices, 0, vertex_count, p1Index);
					...
					for (v = begin; v <= end; v++) {
						...
						Vertex dst = n.vertex[MeshSmooth.CCW(n, vertices.get(v).getIndex())];

						Vertex inv = n.vertex[MeshSmooth.CW(n, vertices.get(v).getIndex())];
						...
						if (inv.face.degree > 2)
							if (MeshSmooth.compare_points(inv.getLocation(), 0,
									p2.getLocation(), 0) == 0) {
								int ni = MeshSmooth.CW(vertices.get(v).face, vertices.get(v).getIndex());
								assert (f.neighbor[i] == null);
								if (n.neighbor[ni] == null) {
