...
public class ProgressDlg extends Dialog {
	...
	public Object open() {
		...
		shell.open();
		shell.layout();
		Vector2f pos = BuilderConfigurationManager.getInstance().getWindowPosition();
		Size2 size = BuilderConfigurationManager.getInstance().getWindowSize();
		shell.setLocation((int)(pos.getX()+size.getWidth()/2 - 100), (int)(pos.getY()+size.getHeight()/2));
				...
				while (shell.isDisposed()==false) {
					...
					shell.getDisplay().asyncExec(new Runnable() {
						...
						public void run() {
							...
							if (BackgroundThreadManager.getInstance().sizeOfThread()==0 && shell.isDisposed()==false)
								shell.dispose();
						}
					});
					try {
