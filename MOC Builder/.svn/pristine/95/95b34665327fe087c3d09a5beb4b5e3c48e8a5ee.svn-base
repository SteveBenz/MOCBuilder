...
public class LDrawPart extends LDrawDrawableElement implements ILDrawObserver,
	...
	public void hitTest(Ray3 pickRay, Matrix4 transform, float scaleFactor,
		...
		if (hidden == false) {
			...
			// model - there is no VBO!
			LDrawDirective modelToDraw = (cacheDrawable == null || boundsOnly) ? cacheModel
					: cacheDrawable;

			if (modelToDraw != null) {
				if (boundsOnly == false) {
					modelToDraw.hitTest(pickRay, combinedTransform,
							scaleFactor, false, creditObject, hits);

				} else {
					LDrawVertices unitCube = LDrawUtilities.boundingCube();
					Box3 bounds = modelToDraw.boundingBox3();
					Vector3f extents = MatrixMath.V3Sub(bounds.getMax(),
							bounds.getMin());
					Matrix4 boxTransform = Matrix4.getIdentityMatrix4();

					// Expand and position the unit cube to match the model
					boxTransform = MatrixMath.Matrix4Scale(boxTransform,
							extents);
					boxTransform = MatrixMath.Matrix4Translate(boxTransform,
							bounds.getMin());

					combinedTransform = MatrixMath.Matrix4Multiply(
							boxTransform, combinedTransform);

					unitCube.hitTest(pickRay, combinedTransform, scaleFactor,
							false, creditObject, hits);
				}
			}
		}
	...
	public void setDisplayName(String newPartName) {
		setDisplayName(newPartName, true, null);
	}
	...
	public void receiveMessage(MessageT msg, ILDrawObservable observable) {
		if (msg == MessageT.MessageNameChanged)
			unresolvePart();
		if (msg == MessageT.MessageScopeChanged)
			unresolvePart();
	}
	...
	public void resolvePart() {
		if (cacheType == PartTypeT.PartTypeUnresolved) {
			LDrawModel mdpModel = referencedMPDSubmodel();
			if (mdpModel != null) {
				cacheModel = mdpModel;
		...
		if (connectivityList == null) {
			...
			if (getCacheType() == PartTypeT.PartTypeSubmodel) {
					...
					for (LDrawDirective directive : step.subdirectives()) {
						if (directive instanceof LDrawPart) {
							...
							for (Connectivity conn : part.getConnectivityList()) {
								conn.setTransformMatrix(Matrix4.multiply(
				...
				connectivityList = ConnectivityLibrary.getInstance()
						.getConnectivity(displayName(), useConnExtractor,
								true);
				if (connectivityList != null) {
...
//									if (matrix[column][row].getAltitude() == 23 || matrix[column][row].getAltitude() == 29)
//										continue;
			...
			if (getCacheType() == PartTypeT.PartTypeSubmodel) {
				...
				for (LDrawStep step : getCacheModel().steps()) {
					for (LDrawDirective directive : step.subdirectives()) {
						if (directive instanceof LDrawPart) {
							LDrawPart part = (LDrawPart) directive;
							for (CollisionBox cbox : part
									.getCollisionBoxList(useCache)) {
								cbox.setTransformMatrix(Matrix4.multiply(
										cbox.getTransformMatrix(),
										part.transformationMatrix()));
								collisionBoxList.add(cbox);
								cbox.setParent(this);
							}
						}
					}
