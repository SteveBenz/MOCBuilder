...
public class CollisionBoxEditorComposite extends ConnectivityEditorComposite {
	 ...
	 * @param style
	 */
	public CollisionBoxEditorComposite(Composite parent, int style) {
		super(parent, style);
		init();
	}
	...
	private void init() {
		...
		group.setLayoutData(new RowData(161, 123));
		...
		if (this.conn == null) {
			...
			CollisionBox cb = (CollisionBox) conn;
			combo_X.select((int) cb.getsX()-1);
			combo_Y.select((int) cb.getsY()-1);
			combo_Z.select((int) cb.getsZ()-1);
	...
	protected void handleGenerate() {
		Vector3f size = new Vector3f(Float.parseFloat(combo_X.getText()),
				Float.parseFloat(combo_Y.getText()), Float.parseFloat(combo_Z
						.getText()));

		CollisionBox newItem = ConnectivityGenerator.getInstance()
				.generateCollisionBox(size);
