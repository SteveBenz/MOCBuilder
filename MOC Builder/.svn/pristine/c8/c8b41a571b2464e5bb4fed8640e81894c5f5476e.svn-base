...
public class BuilderConfigurationManager {
	...
	private String lsynthDirectory;
	...
	private final String DefaultLSynthDirectory;
	...
	private BuilderConfigurationManager() {
		...
		DefaultLSynthDirectory = DefaultLDrawDirectory + "LSynth/";
		...
		lsynthDirectory = DefaultLSynthDirectory;
	...
	private boolean writeToFile() {
		...
		fileContents
				.append("#LSynth Path(String) default: {LDrawPath}/LSynth");
		fileContents.append("\r\n");
		fileContents.append("LSynthDirectory = " + lsynthDirectory);
		fileContents.append("\r\n");
		fileContents.append("\r\n");
		//
	...
	private void parseLine(String line) {
		...
		} else if (line.startsWith("LSynthDirectory")) {
			try {
				String tempString = line.substring(line.indexOf("=") + 1);
				if (tempString.endsWith("/") == false
						&& tempString.endsWith("\\") == false)
					lsynthDirectory = tempString + "/";
				else
					lsynthDirectory = tempString;
				lsynthDirectory = lsynthDirectory.trim();
			} catch (Exception e) {
	...
	public String getLSynthDirectory() {
		return lsynthDirectory;
	}
}
