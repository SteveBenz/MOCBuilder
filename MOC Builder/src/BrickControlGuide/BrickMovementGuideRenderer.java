...
public class BrickMovementGuideRenderer {
	...
	public void draw(GL2 gl2) {
		...
		if (selectedGuide == null && part != null) {			
			if (guideList.size() != 0 && isForGroup==false && BuilderConfigurationManager.getInstance().isUseConnectivity()) {
				synchronized (guideList) {
	...
	private void addGuideForPart(LDrawPart part){
		GlobalConnectivityManager connectivityManager = GlobalConnectivityManager
				.getInstance();
		if (part.isConnectivityInfoExist()) {
			ArrayList<MatrixItem> centerItem = new ArrayList<MatrixItem>();
			if (connectivityManager.isRotatibleByCustom2dConn(part,
					centerItem)) {
				for(MatrixItem item : centerItem)
				BrickMovementGuideRenderer.getInstance()
						.addRotationGuide(
								item.getDirectionVector(Matrix4
										.getIdentityMatrix4()), item);
			}			
			
			for (IConnectivity conn : part.getConnectivityList()) {
				if (connectivityManager.isConnectible_Connectivity(conn.getConnectivity(),
						part.transformationMatrix()).getResultType() != ConnectivityTestResultT.True)
					continue;
				...
				if (conn instanceof Axle || conn instanceof Slider) {
					...
					if (conn instanceof Axle) {
						if(((Axle)conn).gettype()!=4 && ((Axle)conn).gettype()!=5)
							BrickMovementGuideRenderer.getInstance()
