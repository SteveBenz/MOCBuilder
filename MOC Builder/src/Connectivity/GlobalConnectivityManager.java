...
public class GlobalConnectivityManager {
	...
	public Matrix4 getClosestConnectablePos(LDrawPart part,
		...
		if (BuilderConfigurationManager.getInstance().isUseConnectivity()
				...
				&& MatrixMath.compareFloat(hittedWorldPos.y, 0) == 0) {
					...
					if (item.getParent() instanceof Hole) {
						...
						if (minPos == null) {
							...
							minItem = item;
								...
								.add(item.getParent().getParent().position()
										.sub(part.position())).y) {
							minPos = item.getCurrentPos(new Matrix4()).add(
									item.getParent().getParent().position()
											.sub(part.position()));
			...
			if (minPos != null) {
				...
				Vector3f newPosOfMinItem = minItem.getCurrentPos().add(
						testPosOfPart.sub(part.position()));

				Vector3f snappedItemPosDiff = newPosOfMinItem
						.sub(LDrawGridTypeT.getSnappedPos(newPosOfMinItem,
								LDrawGridTypeT.Coarse).add(0, 0,
				...
				newTransform.element[3][1] = -minPos.y;
		...
		// default
		Float minYPos = null;
		Vector3f boundOfPart = null;
		LDrawPart minYPosPart = null;
		LDrawPart testPart=part;
//		for (LDrawPart testPart : BrickSelectionManager.getInstance()
//				.getSelectedPartList()) {
			if (minYPos == null) {
...
//		}
		...
		if (minYPosPart == null)
			return null;
		Matrix4 rotationMatrix = part.transformationMatrix();
		rotationMatrix.element[3][0] = rotationMatrix.element[3][1] = rotationMatrix.element[3][2] = 0;
		boundOfPart = part.boundingBox3(rotationMatrix).getMax();

		minYPos -= part.position().y;

		Vector3f snappedItemPosDiff = boundOfPart.sub(LDrawGridTypeT
				.getSnappedPos(
						boundOfPart,
						BuilderConfigurationManager.getInstance()
								.getGridUnit()).add(
						-LDrawGridTypeT.Coarse.getXZValue() / 2, 0, 0));

		Vector3f snappedPos = LDrawGridTypeT.getSnappedPos(testPosOfPart,
				BuilderConfigurationManager.getInstance().getGridUnit());
		Matrix4 newTransform = part.transformationMatrix();
		newTransform.element[3][0] = snappedPos.x + snappedItemPosDiff.x;
		newTransform.element[3][1] = LDrawGridTypeT.getSnappedPos(
				hittedWorldPos, LDrawGridTypeT.Fine).y - minYPos;

		newTransform.element[3][2] = snappedPos.z + snappedItemPosDiff.z;
		return newTransform;
	}
	...
	public ConnectivityTestResult isConnectable_Exact(LDrawPart part,
		...
		ArrayList<Connectivity> connectivityList = part.getConnectivityList();
	...
	public ArrayList<LDrawPart> getConnectedPart(LDrawPart part,
			IConnectivity centerConn, boolean isForRotation) {
		ArrayList<LDrawPart> resultList = new ArrayList<LDrawPart>();
		getConnectedPart(part, centerConn, isForRotation, resultList);
		return resultList;
	}

	private void getConnectedPart(LDrawPart part, IConnectivity centerConn,
			boolean isForRotation, ArrayList<LDrawPart> resultList) {
		...
		for (Connectivity conn : part.getConnectivityList()) {
			if (centerConn != null) {
				if (centerConn.getConnectivity() == conn)
					continue;
				if (isForRotation == false)
					if (centerConn instanceof Axle && conn instanceof Axle) {
						if (centerConn.getDirectionVector().equals(
								conn.getDirectionVector()))
							continue;
						if (centerConn.getDirectionVector().equals(
								conn.getDirectionVector().scale(-1)))
							continue;
					}

				if ((centerConn instanceof Hinge && conn instanceof Hinge)
						|| (centerConn instanceof Axle && conn instanceof Axle)) {
					if (centerConn.getDirectionVector().equals(
							conn.getDirectionVector())
							|| centerConn.getDirectionVector().equals(
									conn.getDirectionVector().scale(-1))) {
						Vector3f dVector = centerConn.getCurrentPos().sub(
								conn.getCurrentPos());
						dVector = dVector.div(dVector.length());
						if (conn.getDirectionVector().equals(dVector)
								|| conn.getDirectionVector().equals(
										dVector.scale(-1)))
							continue;
					}
				}

				if (centerConn instanceof Ball && conn instanceof Ball)
					continue;
			}
			...
			if (conn.getConnectedConnectivity() != null)
				for (Connectivity eItem : conn.getConnectedConnectivity()) {
					...
					if (eItem.getParent().isSelected())
						continue;
					getConnectedPart(eItem.getParent(), null, isForRotation,
							resultList);
				}
		}
		...
		if (part.getConnectivityMatrixItemList() != null)
			for (MatrixItem matrixItem : part.getConnectivityMatrixItemList()) {
				...
				if (resultList.contains(eItem.getParent().getParent()))
					continue;
				if (centerConn != null && centerConn instanceof MatrixItem) {
					if (centerConn.getDirectionVector().equals(
							eItem.getDirectionVector()))
						continue;
					if (centerConn.getDirectionVector().equals(
							eItem.getDirectionVector().scale(-1)))
						continue;
				}

				getConnectedPart(eItem.getParent().getParent(), null,
						isForRotation, resultList);
			}
	}
	...
	public boolean isRotatibleByCustom2dConn(LDrawPart part,
			ArrayList<MatrixItem> rotationCenter) {

		boolean isRotatible = false;
