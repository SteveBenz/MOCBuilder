...
public class ConnectivityInfoComposite extends Composite {
	 ...
	 * @param style
	 */
	public ConnectivityInfoComposite(Composite parent, int style) {
		super(parent, style);
		setLocation(0, 0);
		setSize(780, 560);
	...
	protected void handleUploadAll() {
		UpdateManager.getInstance().uploadCustomConnectivities();
	...
	protected void handleDownloadAll() {
		UpdateManager.getInstance().downloadNewConnectivities();
		BackgroundThreadManager.getInstance().add(new Runnable(){
			@Override
			public void run() {
				Display.getDefault().asyncExec(new Runnable(){
	...
	protected void updateTableWithKeyword(String keyword, boolean showMappedItem) {
		...
		int index = 0;
		String state = "New";
		for (String partId : connectivityInfoList_server) {
