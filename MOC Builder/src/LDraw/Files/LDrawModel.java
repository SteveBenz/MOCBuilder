...
public class LDrawModel extends LDrawContainer implements Cloneable {
	...
	public LDrawModel initWithLines(ArrayList<String> lines, Range range,
		...
		// #endif
		// Parse out steps. Each time we run into a new 0 STEP command, we
		// finish
		// the current step.
		do {
	...
	public void drawSelf(GL2 gl2, ILDrawRenderer renderer) {
		...
		if (!isOptimized) {
			...
			// ILDrawDLHandle drag_dl = null;
			// LDrawDLCleanup_f drag_dl_dtor = null;
			//
			// ILDrawCollector collector = renderer.beginDL();
			// draggingDirectives.collectSelf(collector);
			// drag_dl = renderer.endDL(gl2, drag_dl_dtor);
			//
			// if (drag_dl != null) {
			// renderer.drawDL(gl2, drag_dl);
			// // todo
			// // drag_dl_dtor(drag_dl);
			// }
			//
			// draggingDirectives.drawSelf(gl2, renderer);
			// }
		}
	}// drawSelf:
