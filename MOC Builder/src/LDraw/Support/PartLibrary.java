...
public class PartLibrary {
	...
	//
	// ==============================================================================
	public void loadModelForName(String partName, DispatchGroup parentGroup) {
		// Determine if the model needs to be parsed.
		// Dispatch to a serial queue to effectively mutex the query
		// #if USE_BLOCKS
		// dispatch_group_async(parentGroup, catalogAccessQueue,
		...
		// Already been parsed?
		model = (LDrawModel) loadedFiles.get(partName);
		if (model == null) {
			// Nobody has started parsing it yet, so we win! Parse from disk.
			String partPath = LDrawPaths.sharedPaths()
					.pathForPartName(partName);
			model = readModelAtPath(partPath, false, null);
			if (model != null) {
				loadedFiles.put(partName, model);
			}
		}
	...
	//
	// ==============================================================================
	public LDrawModel readModelAtPath(String partPath, boolean asynchronous,
			LDrawModel completionBlock) {
		return readModelAtPath(partPath, asynchronous, completionBlock, true);
	}
