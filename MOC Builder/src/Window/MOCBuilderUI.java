...
public class MOCBuilderUI implements GLEventListener, ILDrawSubscriber {
	...
	GroupEditorView fileInfoTreeView;
	...
	public void open(Display display) {
		...
		shell.addDisposeListener(new DisposeListener() {
			...
			public void widgetDisposed(DisposeEvent arg0) {
				...
				BuilderConfigurationManager.getInstance()
						.setWindowPosition(shell.getLocation().x,
								shell.getLocation().y);
		...
		if (AutoSaveManager.getInstance().isExistAutoSaveFile()) {
			...
			messageBox
					.setMessage("Temperary file is existing. \r\n Do you want to load it?");
		...
		// brickBuilder.openFile("j:/untitled.ldr");
	...
	private void generateComposite() {
		...
		// Menu bar
		new MenuHandler(brickBuilder, shell).generateMenu();

		// Toolbar
		toolbar = new ToolBarHandler(brickBuilder, shell);
		...
		// FileInfoTreeView
		fileInfoTreeView = new GroupEditorView(brickBuilder);
		...
		StatusBar statusBar = new StatusBar(shell, SWT.BORDER, brickBuilder);
	...
	public void setDragTarget(Control control) {
		...
		target.addDropListener(new DropTargetListener() {
			...
			public void drop(DropTargetEvent event) {
				brickBuilder.getMetaInfoRenderer().setShowBaseplate(
						BuilderConfigurationManager.getInstance()
				...
				if (part == null) {
						...
						if (fileTransfer.isSupportedType(event.currentDataType)) {
							...
							for (String path : filePaths) {
								if (path.toLowerCase().endsWith(".ldr")
									...
									brickBuilder.importFile(path, pos);
									return;
								}
					...
					if (BuilderConfigurationManager.getInstance()
						...
						brickBuilder.addDirectiveToWorkingFile(part);
			...
			public void dragOver(DropTargetEvent event) {
				if (part != null) {
					...
					Vector3f hitPos = brickBuilder.getHittedPos(coordinates.x,
							coordinates.y, true);

					brickBuilder.moveDirectiveTo(part, hitPos);
				}
			}
			...
			private void removeBrick(LDrawPart part) {
				brickBuilder.removeDirectiveFromWorkingFile(part, true);
			...
			private void addBrick(String partName, DropTargetEvent event) {
				...
				brickBuilder.addDirectiveToWorkingFileForDragAndDrop(part);
					...
					if (object != null) {
						...
						if (browserUI.contains(partName)) {
							...
							if (part.getConnectivityMatrixItemList() == null)
								brickBuilder.getMetaInfoRenderer()
	...
	public void init(GLAutoDrawable glautodrawable) {
		...
		glRenderer = brickBuilder.getGLRenderer();
		metaInfoRenderer = brickBuilder.getMetaInfoRenderer();
		brickMovementGuideRenderer = brickBuilder
				.getBrickMovementGuideRenderer();
		connectivityRenderer = brickBuilder.getConnectivityRenderer();
		brickSelectionInfoRenderer = brickBuilder
				.getBrickSelectionInfoRenderer();
		boundingBoxRenderer = GlobalBoundingBoxRenderer.getInstance(camera);

		brickBuilder.initGLReleatedComponent(new Size2(800f, 600f), gl2);
		// animator.start();
	}
	...
	private void initEventListener() {
		BuilderEventHandler eventHandler = new BuilderEventHandler(
				brickBuilder);
